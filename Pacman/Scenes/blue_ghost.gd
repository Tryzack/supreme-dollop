	...
	if state == 1:
		if levelNavigation != null and player != null:
			...
			if distance < 5 * tile_size:
				...
				var player_heading = player.position + (player.velocity.normalized() * 3 * tile_size)
				var blinky_vector = (red.position - player_heading)
